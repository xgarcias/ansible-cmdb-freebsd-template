...
<%def name="host_zpool(host)">
  % if len(jsonxs(host, 'ansible_facts.ansible_local.zfs.zpool', default={}).items()) != 0:
    <h4 class="toggle-collapse ${collapsed_class}">ZFS pools</h4>
    <div class="collapsable ${collapsed_class}">
      % for zpool in sorted(jsonxs(host, 'ansible_facts.ansible_local.zfs.zpool', default={})):
    <h4>${zpool}</h4>
    <table>
    <tr><th>Health</th><td>${jsonxs(host, 'ansible_facts.ansible_local.zfs.zpool.%s.properties.health' % zpool, default={})}</td></tr>
    <tr><th>Size</th><td>${jsonxs(host, 'ansible_facts.ansible_local.zfs.zpool.%s.properties.size' % zpool, default={})}</td></tr>
    <tr><th>Free</th><td>${jsonxs(host, 'ansible_facts.ansible_local.zfs.zpool.%s.properties.free' % zpool, default={})}</td></tr>
    <tr><th>Capacity</th><td>${jsonxs(host, 'ansible_facts.ansible_local.zfs.zpool.%s.properties.capacity' % zpool, default={})}</td></tr>
    </table>
    </br>
    <table>
    <tr>
      ...
      % for volume in sorted(jsonxs(host, 'ansible_facts.ansible_local.zfs.zpool.%s.volumes' % zpool, default={})):
    <tr>
      <td>${volume}</td>
      <td>${jsonxs(host, 'ansible_facts.ansible_local.zfs.zpool.%s.volumes.%s.type' % (zpool,volume), default={})}</td>
      <td>${jsonxs(host, 'ansible_facts.ansible_local.zfs.zpool.%s.volumes.%s.used' % (zpool,volume), default={})}</td>
      <td>${jsonxs(host, 'ansible_facts.ansible_local.zfs.zpool.%s.volumes.%s.available' % (zpool,volume), default={})}</td>
      <td>${jsonxs(host, 'ansible_facts.ansible_local.zfs.zpool.%s.volumes.%s.compressratio' % (zpool,volume), default={})}</td>
      <td>${jsonxs(host, 'ansible_facts.ansible_local.zfs.zpool.%s.volumes.%s.mountpoint' % (zpool,volume), default={})}</td>
    </tr>
      % endfor
    </table>
      % endfor
